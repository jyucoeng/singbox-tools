...
# 用法如下：
# 1、安装（可覆盖安装，端口号不指定则会随机端口（本版本无用户名/密码认证））：
...
#
#  curl --socks5-hostname "ipv4:端口号" http://ip.sb
#
#  curl -6 --socks5-hostname "[ipv6]:端口号" http://ip.sb
#
...
# 全局常量
########################
INSTALL_DIR="/usr/local/sb-socks5-noauth"
CONFIG_FILE="$INSTALL_DIR/config.json"
BIN_FILE="$INSTALL_DIR/sing-box-socks5-noauth"
LOG_FILE="$INSTALL_DIR/run.log"

SERVICE_NAME="sing-box-socks5-noauth"
...
show_node() {
  ...
  # Extract port from the config file using jq
