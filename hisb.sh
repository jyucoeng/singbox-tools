...
export cdn_host=${cdn_host:-"saas.sin.fan"} # Default CDN host for vless or trojan  cdn.7zz.cn
...
# trpt / hypt / vlpt / vlrt / tupt / anypt / socks5pt
# 只要标记存在，就启用对应协议
if [ -n "${trpt+x}" ]; then
    trp=yes
    agp=yes
...
# 判断：至少启用一个协议
any_proto_enabled() {
    is_yes "$vlr" || is_yes "$vlp" || is_yes "$trp" || is_yes "$hyp" || is_yes "$tup" || is_yes "$anyp" || is_yes "$socksp"
}
...
showmode() {
    blue "  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    green "     Sing-box 一键脚本"
    yellow "     协议: vless/trojan (Argo 选1)"
...
ensure_cloudflared_if_needed() {
    # ✅ 仅当启用 argo=vlpt/trpt 且 agp 存在时才需要 cloudflared
    debug_log "【调试】ensure_cloudflared_if_needed：检查是否需要 cloudflared"
    if { [ "${argo:-}" != "vlpt" ] && [ "${argo:-}" != "trpt" ]; } || [ -z "${agp:-}" ]; then
        debug_log "【调试】ensure_cloudflared_if_needed：未启用 Argo（或未启用 vless/trojan），跳过 cloudflared 下载/安装"
        purple "ℹ️ 未启用 Argo（或未启用 vless/trojan），跳过 cloudflared 下载/安装"
        return 0
    fi
...
ins() {
    ...
    debug_log "【调试】关键参数：argo=${argo:-<空>}，agp=${agp:-<空>}，subscribe=$(get_subscribe_flag 2> /dev/null || echo ${subscribe:-false})，nginx_pt=${nginx_pt:-<空>}，argo_pt=${argo_pt:-<空>}"
    # =====================================================
    # 1. 安装并启动 sing-box
    # =====================================================
    installsb
    ...
    debug_log "【调试】即将判断是否进入 Argo 分支：need_argo=$(need_argo && echo yes || echo no)，agp=${agp:-<空>}"

    # =====================================================
    # 2. Argo 相关逻辑（仅在启用 argo + agp 时）
    # =====================================================
    if need_argo && [ -n "$agp" ]; then
        debug_log "【调试】已进入 Argo 启动分支（argo=${argo}，agp=${agp}）"
        echo
...
regenerate_links_and_sub() {
    ...
    local argodomain cdn_host cdn_pt vlvm vlessws_link1 tratls_link1 sbtk
    ...
    if [ -n "$argodomain" ]; then
        ...
        green "${vlessws_link1}${tratls_link1}"
        append_jh "${vlessws_link1}${tratls_link1}"
        yellow "---------------------------------------------------------"
...
check_port_conflicts_or_exit() {
    ...
    local vars="vlpt trpt vlrt hypt tupt anypt socks5pt"
    if $need_nginx; then
...
menu_reload_proto_flags() {
    trp=; agp=; hyp=; vlp=; vlr=; tup=; anyp=; socksp=
    [ -n "${trpt+x}" ] && { trp=yes; agp=yes; }
    [ -n "${hypt+x}" ] && hyp=yes
    [ -n "${vlpt+x}" ] && { vlp=yes; agp=yes; }
    [ -n "${vlrt+x}" ] && vlr=yes
    [ -n "${tupt+x}" ] && tup=yes
    [ -n "${anypt+x}" ] && anyp=yes
    [ -n "${socks5pt+x}" ] && socksp=yes
    export trp hyp vlp vlr tup anyp socksp agp
    # 重新绑定端口变量（与文件顶部一致）
    export port_vl_ws=${vlpt:-''} port_tr=${trpt:-''} port_hy2=${hypt:-''} \
...
menu_collect_install() {
    local _ans _ch _sel _has_all _has_vless _has_trojan
    ...
    if [ -z "$_ans" ] || [ "$_ans" = "2" ]; then
        ...
        [ -n "$trp$vlp" ] && export argo_pt="$(menu_ask_port "Argo" 8001)"
    else
        green "  ↳ 端口: 全部随机生成"
        ...
        [ -n "$trp$vlp" ] && { argo_pt="${argo_pt:-8001}"; export argo_pt; green "  ↳ Argo 端口: ${argo_pt} (默认)"; }
    fi

    menu_reload_proto_flags

    # Argo 隧道配置（vless/trojan 已强制二选一，这里最多启用一个）
    if [ -n "$vlp" ]; then
        export argo=vlpt
    elif [ -n "$trp" ]; then
        export argo=trpt
...
# 刷新 nginx 配置并重启（trojan/vless/argo 端口变更时需要）
...
edit_ports_menu() {
    ...
    while true; do
        ...
        case "$_sel" in
            ...
            2) _desc="Vless-WS (Argo)"; _file=port_vl_ws; _tag=vless-ws-sb; _need_nginx=1 ;;
    ...
    while true; do
        ...
        case "$_sel" in
            ...
            1)
                ...
                # 本地回源端口不存在时使用默认 8001（避免 JSON 模式因空端口生成失败）
                local _ap
                _ap="$(cat "$SINGBOX_FOLDER_PATH/argoport" 2>/dev/null)"
                [ -n "$_ap" ] || _ap="$ARGO_DEFAULT_PORT"
                echo "$_ap" > "$SINGBOX_FOLDER_PATH/argoport"
                prepare_argo_credentials "$_a" "$_d" "$_ap"
...
# 切换 Argo 使用协议（Trojan-WS-TLS / Vless-WS-TLS）
...
node_config_menu() {
    ...
    while true; do
        ...
        green "  4) Argo 隧道修改"
        green "  5) 切换 Argo 使用协议 (Trojan-WS-TLS / Vless-WS-TLS)"
        purple "  0) 返回主菜单"
        reading "请输入选择: " _ch
        case "$_ch" in
            0) return ;;
...
rt_proxy_map() {
    ...
    local _order=(vless-ws-sb trojan-ws-sb vless-reality-vision-sb hy2-sb tuic-sb anytls-sb)
    clear
...
edit_protocol_proxy() {
    local sbj="$SINGBOX_FOLDER_PATH/sb.json"
    local _letters=(b c d e f g) _ptags=(vless-reality-vision-sb hy2-sb tuic-sb anytls-sb vless-ws-sb trojan-ws-sb)
    ...
    if [ ${#_protos[@]} -eq 0 ]; then
        yellow "未检测到已安装的协议入站（tuic/hy2/vless 等）。"; menu_pause; return
    fi
...
# 编号与安装时一致且按字母升序：a=全选 b=VLESS c=Hysteria2 d=TUIC e=AnyTLS f=Vless-WS g=Trojan-WS
attach_socks5_proxy() {
     local sbj="$SINGBOX_FOLDER_PATH/sb.json"
